...
class LightLatentMarkovDynamics(LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        q_ang = torch.reshape(state_traj[:, :, :12], (-1, 12, 1))
        q_ang = torch.arctan2(q_ang[:, :, 1], q_ang[:, :, 0])
        q_ang_gt = torch.reshape(gt_state_traj[:, :, :12], (-1, 12, 1))
        q_ang_gt = torch.arctan2(q_ang_gt[:, :, 1], q_ang_gt[:, :, 0])

        q_err = torch.mean((q_ang - q_ang_gt) ** 2, dim=0)
        dq_err = mse[12:24]
        z_err = mse[24]
        base_vel_err = mse[25:28]
        base_ori_err = mse[28:31]
        base_ang_vel_err = mse[31:34]

        scalar_metrics.update(state_q_err=torch.mean(q_err), state_dq_err=torch.mean(dq_err),
                              state_z_err=torch.mean(z_err), state_base_vel_err=torch.mean(base_vel_err),
    ...
    def on_test_end(self) -> None:
        ...
        if isinstance(linear_dynamics, EquivLinearDynamics):
            ...
            iso_koop_operators = {}
            for field_start, field_end, field_rep in zip(model_state_type.fields_start,
                                                         model_state_type.fields_end,
                                                         model_state_type.representations):
            ...
            for iso_irrep_id, koop_op in iso_koop_operators.items():
                ...
                iso_df_eigvals = pd.DataFrame({"real":  eig_vals.real,
                                               "imag":  eig_vals.imag})
            ...
            df_eigvals = pd.DataFrame({"real":  eig_vals.real,
                                       "imag":  eig_vals.imag})
            df_eigvals = df_eigvals.assign(irrep=None)
            wandb_eigvals_table = wandb.Table(dataframe=df_eigvals)

        wandb_logger.log({f"trans_op_eigvals":  wandb_eigvals_table,
        ...
        if self.trainer.current_epoch % log_vector_metrics_n_steps == 0 or flush:
            ...
            for metric_log_name, metric_vector in self._log_cache.items():
                if "_t/" in metric_log_name:  # Vector to be plotted against time
                    assert metric_vector.ndim == 2, (f"{metric_log_name} Expected (batch, time_steps) vector, "
                                                     f"got {metric_vector.shape}.")
