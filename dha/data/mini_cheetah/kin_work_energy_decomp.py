...
from DynamicsHarmonicsAnalysis.dha.data.DynamicsRecording import DynamicsRecording, get_train_test_val_file_paths
import morpho_symm
...
if __name__ == "__main__":
    ...
    for recording_name in recordings:
        ...
        iso_joint_angle_t_iso_basis, iso_joint_angle_t_ori_basis = decom_signal_into_isotypic_components(joint_angle_t,
                                                                                                         rep_TqQjs)
        ...
        iso_space_names = {iso_irrep_id: r"$\mathbb{G}^{(%d)}$" % i for i, iso_irrep_id in
                           enumerate(iso_joint_vel_iso_basis.keys())}
        ...
        work_df_long = work_dist_df.melt(id_vars=['time'],
                                         value_vars=ordered_iso_comps,
                                         var_name='Source',
                                         value_name='Work')
        ...
        fig = make_subplots(rows=len(kin_energy_iso_decomp_t),
                            cols=1,
                            shared_xaxes=True)
        ...
        for iso_comp_id, energy_t in kin_energy_iso_decomp_t.items():
            ...
            fig.add_trace(go.Scatter(x=np.asarray(time_window) * dt,
                                     y=energy_t,
                                     mode='lines',
