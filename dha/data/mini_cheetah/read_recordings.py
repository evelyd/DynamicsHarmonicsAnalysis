...
def convert_mini_cheetah_raysim_recordings(data_path: Path):
    ...
    train_recording, val_recording, test_recording = split_train_val_test(data_recording)

    # Save each split to a file with a corresponding name
    train_file_name = (f"n_trajs={train_recording.info['num_traj']}"
                       f"-frames={train_recording.info['trajectory_length']}-train.pkl")
    val_file_name = (f"n_trajs={val_recording.info['num_traj']}"
                     f"-frames={val_recording.info['trajectory_length']}-val.pkl")
    test_file_name = (f"n_trajs={test_recording.info['num_traj']}"
                      f"-frames={test_recording.info['trajectory_length']}-test.pkl")

    train_recording.save_to_file(data_path.parent.parent / train_file_name)
    val_recording.save_to_file(data_path.parent.parent / val_file_name)
    test_recording.save_to_file(data_path.parent.parent / test_file_name)

    print(f"Train Dynamics Recording saved to {data_path.parent.parent / train_file_name}")
    print(f"Validation Dynamics Recording saved to {data_path.parent.parent / val_file_name}")
    print(f"Test Dynamics Recording saved to {data_path.parent.parent / test_file_name}")
